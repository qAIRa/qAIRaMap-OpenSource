...
const requestOptions = async (element) => {
  ...
  await noParametersRequest("AllQhawaxInMap/").then((q) =>
    q.forEach(async (qhawax) => {
      await oneParameterRequest("qhawax_status/?name=", qhawax.name).then(
        (s) =>
          s === "ON" &&
          addOptions.appendChild(
...
const createTraces = async (time, qhawax) => {
  ...
  let x = [];
  await requestGraphicsData(qhawax, time).then((g) =>
    g.forEach((d) => {
