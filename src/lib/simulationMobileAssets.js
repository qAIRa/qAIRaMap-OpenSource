...
import { addLine, removeLine, newPolyline, newCircle} from './droneAssets.js';
...
export const callOnceMarker = (flag0, trip, map) => {
  ...
  export const drawSensorValues = async (trip,map) => {
    ...
    if(data.length>0){
      ...
      toast(`There are no measurements for ${trip.name} on sensor ${trip.sensor}`,'grey darken-1 rounded')
     }
  }
